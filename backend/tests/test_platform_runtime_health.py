...
def test_runtime_health_skips_embed_default_for_chat_probe(
    monkeypatch,
) -> None:
    monkeypatch.delenv("TEAM_AI_PLATFORM_ADMIN_TOKEN", raising=False)
    monkeypatch.delenv("TEAM_AI_PLATFORM_DB_DSN", raising=False)
    ...
    def fake_call_json(
        ...
        *,
        ...
        if url.endswith("/v1/chat/completions"):
            model = str((payload or {}).get("model", ""))
            called_chat_models.append(model)
            return 200, {"choices": [{"message": {"content": "ok"}}]}
    ...
    with TestClient(app) as client:
        ...
        assert called_chat_models == ["deepseek-v4-flash"]
        assert checks_by_name["chat_probe"]["ok"] is True
        assert "fallback=" not in checks_by_name["chat_probe"]["detail"]
